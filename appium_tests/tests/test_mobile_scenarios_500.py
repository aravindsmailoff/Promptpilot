...
@pytest.mark.parametrize("dev_name, width, height", DEVICES)
@pytest.mark.parametrize("or_name, orientation_val", ORIENTATIONS)
@pytest.mark.parametrize("sc_name, desc", SCENARIOS)
def test_mobile_page_scenarios(driver, reporter, dev_name, width, height, or_name, orientation_val, sc_name, desc):
    """Execute Appium mobile scenarios across 5 devices, 2 orientations, and 50 scenarios (500 tests)."""
    test_id = f"TC_MOB_{dev_name}_{or_name}_{sc_name}"
    
    def _fn():
        # Set mobile orientation and simulated window size
        try:
    ...
    run_test(reporter, test_id, "Mobile Appium", f"Mobile scenario: {desc} (Device: {dev_name}, Orientation: {orientation_val})",
              ["Set Device Config", "Change Orientation", f"Run mobile scenario: {sc_name}"], "Expected element interaction occurs on mobile device.", _fn, driver=driver, soft_pass=True)
