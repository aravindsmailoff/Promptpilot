"""Core smoke E2E tests TC_001 – TC_009 (sequential integration flows) for Appium."""
import sys
import time
...
def test_tc_002_google_login(driver, reporter):
    def _fn():
        ...
        login_page.click_settings_tab()
        time.sleep(2)
        if login_page.is_logged_in():
            info = login_page.get_logged_in_user_info()
            return f"Authenticated on mobile. Email: {info['email']}, Name: {info['name']}"
    ...
    run_test(reporter, "TC_002", "Authentication",
             ...
             [f"Navigate mobile Settings", "Inspect session"],
...
def test_tc_003_execute_mission_and_db_verify(driver, reporter, db_helper):
    def _fn():
        login_page = AppLoginPage(driver)
        dashboard_page = AppDashboardPage(driver)
        unique_task = f"Appium Automated Test Mission Run {int(time.time())}"
        ...
        try:
            ...
            return f"Mission submitted. Task: {unique_task[:50]}"
...
def test_tc_004_submit_model_and_db_verify(driver, reporter, db_helper):
    def _fn():
        ts = int(time.time())
        model_name = f"AppiumTestModel_{ts}"
        model_url = f"https://appium-test-model-{ts}.ai"
        login_page = AppLoginPage(driver)
        dashboard_page = AppDashboardPage(driver)
...
def test_tc_005_startup_and_services_verification(reporter):
    def _fn():
        ports = {8001: "FastAPI Context Server", 8002: "WhatsApp Service", 9002: "Next.js Dev Server", 4723: "Appium Server"}
        active, offline = [], []
...
def test_tc_006_cofounder_mode(driver, reporter):
    def _fn():
        ...
        dashboard_page.switch_to_rd_mode()
        return "Co-Founder profile activated on mobile."
    ...
    def _fn():
        ...
        if not login_page.is_logged_in():
            try:
                ...
                pass
            return "Security Lock screen verified."
        return "Mission Log header visible."
...
def test_tc_008_context_pilot_flow(driver, reporter):
    def _fn():
        context_page = AppContextPilotPage(driver)
        ...
        unique = f"Appium memory {int(time.time())} secret code Antigravity"
...
# ── Full-suite re-export (for runners that only collect test_flow.py) ──────────
import importlib

for _mod_name in ("test_unit", "test_functional", "test_ui_ux", "test_validation", "test_deployable"):
    _mod = importlib.import_module(_mod_name)
    for _name in dir(_mod):
        if _name.startswith("test_") and _name not in globals():
