...
# Load environment to check NEXTAUTH_SECRET
from dotenv import load_dotenv
env_paths = [Path(__file__).parent / ".env.local", Path(__file__).parent / ".env"]
...
allowed_origins = ["http://127.0.0.1:3000", "http://localhost:3000", "http://127.0.0.1:9002", "http://localhost:9002"]
if NEXTAUTH_URL:
    allowed_origins.append(NEXTAUTH_URL)
...
async def verify_authorization(request: Request, call_next):
    ...
    if not auth_header:
        return Response(content='{"error": "Unauthorized: Missing Authorization header"}', status_code=401, media_type="application/json")
        
    parts = auth_header.split(" ")
    if len(parts) != 2 or parts[0].lower() != "bearer":
        return Response(content='{"error": "Unauthorized: Invalid Authorization header format"}', status_code=401, media_type="application/json")
        
    token = parts[1]
    if not NEXTAUTH_SECRET or token != NEXTAUTH_SECRET:
        return Response(content='{"error": "Unauthorized: Invalid security token"}', status_code=401, media_type="application/json")
