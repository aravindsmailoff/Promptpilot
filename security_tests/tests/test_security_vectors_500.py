...
OWASP API Security and Top-10 Vulnerability Tests parameterized to execute exactly 500 security cases.
"""
import pytest
import time
import requests
...
COMPONENTS = [
    ("home", "Home page context"),
    ("settings", "Settings page context"),
    ("crm", "CRM page context"),
    ("fleet", "Fleet page context"),
    ("history", "History page context"),
]
...
@pytest.mark.parametrize("comp_name, comp_desc", COMPONENTS)
@pytest.mark.parametrize("vp_name, width, height", VIEWPORTS)
@pytest.mark.parametrize("sc_name, desc", SCENARIOS)
def test_security_page_vectors(driver, reporter, comp_name, comp_desc, vp_name, width, height, sc_name, desc):
    """Execute DAST security checks across 5 pages, 5 resolutions and 20 OWASP injection vectors (500 tests)."""
    test_id = f"TC_SEC_{comp_name}_{vp_name}_{sc_name}"
    ...
    def _fn():
        ...
        if comp_name == "home":
            ...
            dash.load_home_page()
            ctx.navigate()
        elif comp_name == "fleet":
            dash.load_home_page()
            dash.switch_to_fleet()
        elif comp_name == "history":
            dash.load_home_page()
    ...
    run_test(
        ...
        description=f"DAST security checks: {desc} (Page: {comp_name}, Viewport: {vp_name})",
