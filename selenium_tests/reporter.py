...
from config import REPORT_FILE_PATH, SCREENSHOTS_DIR

class TestReporter:
    """Handles compilation of test results, taking screenshots, and exporting a stylized Excel report."""
    ...
    def generate_excel_report(self):
        ...
        title_cell = ws["A1"]
        title_cell.value = "PROMPT PILOT — END-TO-END AUTOMATION TEST REPORT"
        ...
        meta_cell = ws["A2"]
        meta_cell.value = f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | Target Environment: {config_target_url()}"
...
def config_target_url():
    """Helper to fetch config URL safely without circular imports."""
