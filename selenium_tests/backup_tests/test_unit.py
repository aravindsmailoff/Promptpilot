"""Unit Tests UT_001 – UT_026 (no browser required)."""
import os
import sys
import json
import re
...
def _import_context_server():
    ...
    # Prevent heavy model load side effects by mocking if needed
    try:
        spec.loader.exec_module(mod)
    except Exception:
        # Fallback: define minimal parsers inline for unit tests
        return None
    return mod
...
def test_ut_014_search_empty_query(reporter):
    def _fn():
        ...
        r = requests.post("http://127.0.0.1:8001/search", json={"query": ""}, headers=config.AUTH_HEADERS, timeout=3)
